...
public class MemberController {
	...
	@RequestMapping(value="/updatePhoneAndEmail",
			method=RequestMethod.POST,
			consumes="application/json")
	public @ResponseBody Map <?,?> updatePhoneAndEmail(@RequestBody Map<String,Object> paramMap) throws Exception{
		logger.info("MemberController-updatePhoneAndEmail() {}","ENTER");
		Map <String,Object> map = new HashMap<>();
		String id = (String) paramMap.get("id");
		String email = (String) paramMap.get("email");
		String phoneNo = (String) paramMap.get("phoneNo");
		...
		paramMap.put("email", email);
		paramMap.put("phoneNo", phoneNo);
		int check=memberService.updatePhoneAndEmail(paramMap);
		if(check==1){
			System.out.println("==========맵퍼를 거치고 들어왔음===========");
			map.put("result", "success");
		}else{
