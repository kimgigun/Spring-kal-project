...
public class MemberService {
	...
	public Member memberInfo(Map<String,Object> paramMap) throws Exception{
		logger.info("MemberService-memberInfo() {}","ENTER");
		IGetService service = (map)->mapper.findMemberInfo(map);
		return (Member) service.execute(paramMap);
	}
	public int updatePhoneAndEmail(Map<String,Object> paramMap) throws Exception{
		logger.info("MemberService-updatePhoneAndEmail() {}","ENTER");
		System.out.println("MemberService로 넘어온 value:"+paramMap.get("phoneNo") + paramMap.get("email"));
		IPutService service = (map)->mapper.updatePhoneAndEmail(map);
		return (int) service.execute(paramMap);
	}
}